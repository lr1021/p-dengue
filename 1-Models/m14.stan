...
model {
    ...
    eps_std ~ exponential(1);
    ni_eps ~ normal(0,1);
    i_eps ~ normal(0,1);

    real combined_mean = (sum(ni_eps) + sum(i_eps)) / (ni_n + i_n);
    combined_mean ~ normal(0, 1.0/(ni_n + i_n));
    ...
    target += log_sum_exp(log_theta,
                          log1m_theta + neg_binomial_2_log_lpmf(0 | beta_0 + ni_log_pop[ni_zeros[n]] + eps_std*ni_eps[ni_zeros[n]], disp));
    }
    target += ni_n_non_zeros * log1m_theta;
    target += neg_binomial_2_log_lpmf(ni_y[ni_non_zeros] | beta_0 + ni_log_pop[ni_non_zeros] + eps_std*ni_eps[ni_non_zeros], disp);
    //

    // isolated
    for (n in 1:i_n_zeros){
    target += log_sum_exp(log_theta,
                          log1m_theta + neg_binomial_2_log_lpmf(0 | beta_0 + i_log_pop[i_zeros[n]] + eps_std*i_eps[i_zeros[n]], disp));
    }
    target += i_n_non_zeros * log1m_theta;
    target += neg_binomial_2_log_lpmf(i_y[i_non_zeros] | beta_0 + i_log_pop[i_non_zeros] + eps_std*i_eps[i_non_zeros], disp);
    //
}