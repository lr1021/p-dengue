...
model {
    ...
    target += log_sum_exp(log_theta,
                          log1m_theta + neg_binomial_2_log_lpmf(0 | beta_0 + ni_log_pop[ni_zeros[n]] + ni_log_surv[ni_zeros[n]], disp));
    }
    target += ni_n_non_zeros * log1m_theta;
    target += neg_binomial_2_log_lpmf(ni_y[ni_non_zeros] | beta_0 + ni_log_pop[ni_non_zeros] + ni_log_surv[ni_non_zeros], disp);
    //

    // isolated
    for (n in 1:i_n_zeros){
    target += log_sum_exp(log_theta,
                          log1m_theta + neg_binomial_2_log_lpmf(0 | beta_0 + i_log_pop[i_zeros[n]] + i_log_surv[i_zeros[n]], disp));
    }
    target += i_n_non_zeros * log1m_theta;
    target += neg_binomial_2_log_lpmf(i_y[i_non_zeros] | beta_0 + i_log_pop[i_non_zeros] + i_log_surv[i_non_zeros], disp);
    //
}