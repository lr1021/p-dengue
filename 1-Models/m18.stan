...
data {
    ...
    vector[ni_n] ni_urb_dens_X_u;
    vector[i_n] i_urb_dens_X_u;
...
model {
    ...
    target += log_sum_exp(log_theta,
                          ...
                          + ni_log_surv[ni_zeros[n]] + beta_urb*ni_urb[ni_zeros[n]] +
                          beta_d*ni_urb_dens_X_u[ni_zeros[n]], disp));
    }
    target += ni_n_non_zeros * log1m_theta;
    target += neg_binomial_2_log_lpmf(ni_y[ni_non_zeros] | beta_0 + ni_log_pop[ni_non_zeros]
                                      + ni_log_surv[ni_non_zeros] + beta_urb*ni_urb[ni_non_zeros]
                                      + beta_d*ni_urb_dens_X_u[ni_non_zeros], disp);
    //
    ...
    target += log_sum_exp(log_theta,
                          ...
                          + i_log_surv[i_zeros[n]] + beta_urb*i_urb[i_zeros[n]]
                          + beta_d*i_urb_dens_X_u[i_zeros[n]], disp));
    }
    target += i_n_non_zeros * log1m_theta;
    target += neg_binomial_2_log_lpmf(i_y[i_non_zeros] | beta_0 + i_log_pop[i_non_zeros]
                                      + i_log_surv[i_non_zeros] + beta_urb*i_urb[i_non_zeros]
                                      + beta_d*i_urb_dens_X_u[i_non_zeros], disp);
    //
}