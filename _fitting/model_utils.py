...
def model_fit_Bdropcentred(data, data_name,
                           model_settings,
                           outpath, task,
    ...
    if check_report and os.path.exists(report_file):
        print(f"Skipping {model_name}, report already exists.")
        return
    if model_builder == 'choose_Bdropcentred':
        model, model_B, model_knot_list = build_model_choose_Bdropcentred(data.copy(), **model_settings)
    elif model_builder == 'choose_Bdropcentred_sigma_w_halft':
        model, model_B, model_knot_list = build_model_choose_Bdropcentred_sigma_w_halft(data.copy(), **model_settings)
    with model:
        if check_idata and os.path.exists(idata_file):
            print(f"Skipping {model_name} idata compute, already exists.")
            idata = az.from_netcdf(idata_file)
...
def build_model_choose_Bdropcentred_sigma_w_halft_zerotemp(data, stat_names, zero_stat_name,
                                                        ...
                                                        sigma_w_nu, sigma_w_sigma,
                                                        intercept_sigma, intercept_mu,
...
def build_model_choose_Bdropcentred(data, stat_names,
                                    disp_sigma, beta_u_sigma, sigma_w_sigma,
                                    intercept_sigma, intercept_mu,
                                    degree=3, num_knots = 3,
                                    knot_type='quantile',
                                    surveillance_name=None, 
...
def build_model_choose_Bdropcentred_sigma_w_halft(data, stat_names,
                                                  disp_sigma, beta_u_sigma,
                                                  sigma_w_nu, sigma_w_sigma,
                                                  intercept_sigma, intercept_mu,
                                                  degree=3, num_knots = 3,
