...
Folder spline_tests contains output of model fits:
- full_drop, unit_nonunit, sigma_w_sd: these show that drop (which removes zero vector (zero row sum)) improves fitting by removing correlation, given the non Linearly Independent columns, centred is really good at removing interaction with baseline and removes need for mean zero constraints as span of spline basis is also centred
weatherstats_NB_compare: show that spline implementation runs, compares B_drop_centred_unit with Q_drop_centred_unit. Unit makes everything comparable, after QR algorithm, the curves are similar. Q fits faster but the unprincipled implied covariance for the weights is a bit iffy. Proceed with B_drop_centred_unit